...
if ${DOCKER} buildx inspect knitfab > /dev/null 2>&1 ; then
	:
else
	${DOCKER} buildx create --name knitfab --node knitfab --driver docker-container
fi
...
for A in ${ARCH} ; do
	...
	ARCH=${A} PLATFORM=${PLATFORM} ${DOCKER} buildx bake --load \
		-f ${ROOT}/build/docker-compose.yml ${BUILDX_SET} \
		${TARGET}

	for COMP in ${COMPONENT} ; do
		echo "${COMP}	${COMP}${SUFFIX}:${APP_VERSION}-${A}"
	done
done
